...
static int eta6953_inform_charger_type(struct eta6953 *eta)
{
	int ret = 0;
...
static irqreturn_t eta6953_irq_handler(int irq, void *data)
{
	...
	if (!ret && prev_chg_type != eta->psy_usb_type && eta->chg_det_enable)
		eta6953_inform_charger_type(eta);
				 ...
				 const struct i2c_device_id *id)
{
	...
	if (eta->part_no == *(int *)match->data && eta->e_part_no == EXTRA_ETA6953 &&
		eta->reg0c_no == REG0C_ETA6953)
	{
		pr_info("part match, hw:%s, devicetree:%s, extra part no:%d, reg0c_no=%d\n",
			pn_str[eta->part_no], pn_str[*(int *) match->data], eta->e_part_no, eta->reg0c_no);
	} else {
		pr_info("part no match, hw:%s, devicetree:%s, extra part no:%d, reg0c_no=%d\n",
			pn_str[eta->part_no], pn_str[*(int *) match->data], eta->e_part_no, eta->reg0c_no);
		eta6953_charger_remove(client);
		return -EINVAL;
	}
